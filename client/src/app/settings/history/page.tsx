...
export default function HistoryPage() {
  ...
  useEffect(() => {
    ...
    // Syncs parameter changes directly on the active sub-route level
    router.replace(
      params.toString() ? `${pathname}?${params.toString()}` : pathname,
      { scroll: false },
    );
  }, [projectId, category, startDate, endDate, router, pathname]);
    ...
    const loadHistory = async () => {
      ...
      try {
        const response = await api.get<HistoryResponse>("/users/me/history", {
          params: { page: 1, limit, ...queryParams },
        });
        ...
        if (active) setError("Failed to load your history.");
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const response = await api.get<HistoryResponse>("/users/me/history", {
        params: { page: nextPage, limit, ...queryParams },
      });
  ...
  const projectOptions = useMemo(() => {
    ...
    entries.forEach((entry) => {
      if (entry.project) map.set(entry.project.id, entry.project.name);
    });
  ...
  const exportHistory = async (format: "json" | "markdown") => {
    try {
      ...
      downloadFile(
        response.data,
        `tracevault-history-${new Date().toISOString().slice(0, 10)}.${format === "markdown" ? "md" : "json"}`,
      );
    } catch (err) {
    ...
    >
      ...
      >
        <div>
          ...
          >
            Filter and export your historical entry events.
          </p>
        </div>
      ...
      {!loading && !error && (
        <>
          ...
          {/* Chronological Vertical Log Streams */}
          <section
            style={{ position: "relative", paddingLeft: 18, marginTop: "8px" }}
          >
            <div
              style={{
                position: "absolute",
