...
function CollectionDetailPageContent() {
  ...
  const handleDeleteCollection = async () => {
    if (!confirm(`Permanently delete collection "${collection?.name}"?`))
      return;
    try {
  ...
  if (loading)
    return (
      <AppLayout title="Collection" subtitle="Loading...">
        ...
        >
          <Loader2 className="animate-spin" color="var(--amber)" />
        </div>
      </AppLayout>
    );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
              <div
                style={{
                  ...
                  height: "56px",
                  borderRadius: "16px",
                  background: "rgba(189, 147, 85, 0.1)",
                  display: "flex",
                  alignItems: "center",
          ...
          >
            ...
            >
              {collection.entryCount} SECTIONS SEALED
            </span>
            <button
        ...
        >
          {collection.entries.length === 0 ? (
            ...
            >
              <div
                style={{ fontSize: "40px", marginBottom: "24px", opacity: 0.5 }}
              >
                Empty
              </div>
              <p
              ...
              <p style={{ fontSize: "14px", marginBottom: "32px" }}>
                Start grouping your engineering wisdom by adding entries from
                the community feed or your own vault.
              </p>
              <button
                onClick={() => setIsAddingEntry(true)}
                style={{
            ...
            collection.entries.map((entry, idx) => (
              <div
                ...
                style={{
                  padding: "24px 32px",
                  borderBottom:
                    idx === collection.entries.length - 1
                      ? "none"
              ...
              >
                <div
                  style={{
                    ...
                    borderRadius: "50%",
                    background:
                      TEMPLATE_COLORS[entry.template_type] ||
                      TEMPLATE_COLORS.general,
                    boxShadow: `0 0 8px ${TEMPLATE_COLORS[entry.template_type] || TEMPLATE_COLORS.general}44`,
                  }}
                />
                <div style={{ flex: 1 }}>
                  <div
                ...
                >
                  <span style={{ fontSize: "18px" }}>
                    {entry.topReactionEmoji || ""}
                  </span>
                  <span
                    style={{
                      fontSize: "10px",
                      color: IMPACT_COLORS[entry.impact],
                      fontWeight: 800,
                      textTransform: "uppercase",
                      fontFamily: "var(--mono)",
                      padding: "4px 8px",
                      borderRadius: "4px",
                      background: `${IMPACT_COLORS[entry.impact]}11`,
                    }}
                  >
                    {entry.impact}
                  </span>
      ...
      <style jsx>{`
        ...
        .entry-row:hover .remove-btn:hover {
          color: var(--rust);
        }
        @keyframes fadeUp {
          from {
...
function AddEntryModal({
  ...
  const handleToggle = async (entryId: string) => {
    try {
      ...
      if (myEntries.includes(entryId))
        setMyEntries(myEntries.filter((id) => id !== entryId));
    ...
    >
      <div
        style={{
          ...
          borderRadius: "32px",
          padding: "40px",
          boxShadow: "0 32px 128px rgba(0,0,0,0.4)",
          position: "relative",
      ...
      >
        ...
        >
          Search and select entries to seal into this collection.
        </p>
            ...
            >
              <p style={{ color: "var(--muted)", fontSize: "13px" }}>
                {query
                  ...
                  : "Type to search your repository"}
              </p>
            </div>
          ) : (
            entries.map((e) => {
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    gap: "16px",
                    cursor: "pointer",
                    background: isIn ? "rgba(42, 107, 94, 0.03)" : "#fff",
                    borderColor: isIn ? "var(--teal)" : "var(--border)",
                    transition: "all 0.2s",
                  }}
                >
                  <div
                    style={{
                      ...
                      borderRadius: "50%",
                      background:
                        TEMPLATE_COLORS[e.template_type] ||
                        TEMPLATE_COLORS.general,
                    }}
                  />
                  <div style={{ flex: 1 }}>
