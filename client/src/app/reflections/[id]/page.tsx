...
export default function ReflectionDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchReflection = async () => {
      ...
      try {
        const response = await api.get<ReflectionDetail>(
          `/reflections/${params.id}`,
        );
        if (isActive) setReflection(response.data);
      } catch (error) {
        ...
        if (status === 403)
          message =
            "This entry is private. Log in with the owner account to view it.";
        else if (status === 404)
  ...
  const templateLabel = useMemo(() => {
    ...
    return (
      TEMPLATE_LABELS[reflection.template_type || reflection.category] ||
      reflection.category
    );
  }, [reflection]);
  ...
  const confidence = useMemo(
    () => getConfidenceBadge(reflection?.fields),
    [reflection],
  );
    ...
    return (
      ...
      >
        ...
        <p style={{ fontSize: 14, color: "var(--muted)" }}>
          {errorMessage ||
            "This entry might be private, deleted, or you may not have access."}
        </p>
        {toast && (
          <Toast
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
        ...
        >
          {/* Top row: badges left, edit/delete icons right */}
          <div
            style={{
              display: "flex",
              alignItems: "center",
          ...
          >
            ...
            <ReflectionDetailClient reflection={reflection} variant="actions" />
          </div>
        ...
        {/* Reactions + Vault/Collection — below main card, same left column */}
        <ReflectionDetailClient
          reflection={reflection}
          variant="bottom-actions"
        />
      </div>

      {/* ── Right: sidebar — only "More from this project" ── */}
      <div style={{ position: "sticky", top: 24 }}>
        <ReflectionDetailClient reflection={reflection} variant="sidebar" />
      </div>
